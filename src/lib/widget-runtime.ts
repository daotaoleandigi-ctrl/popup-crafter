/**
 * Self-contained popup widget runtime, exported as a string so it can be
 * inlined directly into an embed <script> tag. No external file hosting
 * required — the popup renders purely from the data-config base64 payload.
 *
 * Two display modes (config.displayMode):
 *   - "popup": overlay auto-opens on page load (with optional delay).
 *              Exposes window.ScratchPopup.open() + click delegation for
 *              .scratch-popup-trigger / [data-scratch-popup] elements.
 *   - "embed": inline section rendered at the script's location. No overlay,
 *              no auto-open, no close button. Cào → form → cảm ơn in place.
 */

export const WIDGET_RUNTIME = String.raw`(function(){
"use strict";
...
function bindSteps(shadow,host,config,mode,reopenFn){
  var leftCol=shadow.querySelector(".pb-left"),step=1,completed=false,autoCloseTimer=null;
...
b.innerHTML='<div style="display:flex;align-items:center;justify-content:center;width:'+size+'px;height:'+size+'px;border-radius:9999px;background:'+esc(config.bubbleBgColor||"#7c3aed")+';color:'+esc(config.bubbleTextColor||"#fff")+';font-size:'+Math.max(16,size*0.32)+'px;box-shadow:0 8px 28px -8px rgba(0,0,0,.4);">\u{1F381}</div><span class="pb-bubble-label" style="position:absolute;top:50%;'+(pos==="right"?"right:100%;margin-right:8px;":"left:100%;margin-left:8px;")+'transform:translateY(-50%) translateX('+(pos==="right"?"8px":"-8px")+');opacity:0;white-space:nowrap;background:'+esc(config.bubbleBgColor||"#7c3aed")+';color:'+esc(config.bubbleTextColor||"#fff")+';font-size:'+(config.bubbleFontSize||14)+'px;font-family:'+esc(config.bubbleFontFamily||config.fontFamily||"system-ui,sans-serif")+';font-weight:600;padding:6px 14px;border-radius:9999px;box-shadow:0 8px 20px -6px rgba(0,0,0,.4);pointer-events:none;transition:opacity .25s ease,transform .25s ease;">'+esc(label)+'</span>';
  ...
  if(scratchCanvas){var step1=shadow.querySelector(".pb-step-1");if(step1){step1.style.touchAction="none";step1.style.cursor="grab";}initScratch(scratchCanvas,step1||scratchCanvas,config,function(){if(completed)return;completed=true;fireConfetti(confettiCanvas,shadow.querySelector(".pb-scratch-wrap"));var ctx=scratchCanvas.getContext("2d");ctx.clearRect(0,0,scratchCanvas.width,scratchCanvas.height);var claim=shadow.querySelector(".pb-claim"),hint=shadow.querySelector(".pb-hint");if(hint)hint.style.display="none";if(claim)claim.style.display="flex";});}
  ...
  if(useBubble){
    document.addEventListener("click",function(e){
      var t=e.target;
      if(t.closest&&t.closest(".scratch-popup-bubble")){hideBubble();setStep(1);completed=false;if(reopenFn)reopenFn(shadow,config);}
    },true);
  }
...
function mountPopup(config){
  ...
  function reopenFn(s,cfg){
    var sc=s.querySelector(".pb-scratch"),cc=s.querySelector(".pb-confetti");
    if(sc){var s1=s.querySelector(".pb-step-1");initScratch(sc,s1||sc,cfg,function(){fireConfetti(cc,s.querySelector(".pb-scratch-wrap"));var x=sc.getContext("2d");x.clearRect(0,0,sc.width,sc.height);var cl=s.querySelector(".pb-claim"),hi=s.querySelector(".pb-hint");if(hi)hi.style.display="none";if(cl)cl.style.display="flex";});}
  }
  var cleanup=bindSteps(shadow,host,config,"popup",reopenFn);
...
if(!config)return;
if(config.displayMode==="embed"){var h=document.createElement("div");h.style.cssText="display:block;width:100%";me.parentNode.insertBefore(h,me);var sh=h.attachShadow({mode:"open"});sh.innerHTML=styles(config,"embed")+markup(config,"embed");bindSteps(sh,h,config,"embed",null);return;}
